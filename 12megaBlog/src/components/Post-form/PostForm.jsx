...
function PostForm({ post }) {
  ...
  const submit = async (data) => {
    if (post) {
      const file = data.images[0]
        ? configService.uploadFile(data.images[0])
        : null;
      ...
      if (file) {
        ...
        console.log("featuredImage", fileId)
        const dpPost = await configService.createPost({
          ...data,
          userId: userData.$id,
        });
        if (dpPost) {
  ...
  useEffect(() => {
    const subscirption = watch((value, { name }) => {
      if (name === "title") {
